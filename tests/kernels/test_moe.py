...
def test_mixtral_moe(dtype: torch.dtype):
    ...
    # Load the weights
    aphrodite_moe.gate.linear_weights["weight"][:] = hf_moe.gate.weight.data
    for i in range(config.num_local_experts):
        weights = (hf_moe.experts[i].w1.weight.data,
                   hf_moe.experts[i].w3.weight.data)
        aphrodite_moe.ws[i][:] = torch.cat(weights, dim=0)
        aphrodite_moe.w2s[i][:] = hf_moe.experts[i].w2.weight.data
