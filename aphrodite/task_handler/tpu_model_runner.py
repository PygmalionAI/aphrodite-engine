...
class TPUModelRunner:
    ...
    def execute_model(
        ...
        assert seq_group_metadata_list is not None
        if seq_group_metadata_list[0].is_prompt:
            # NOTE: To reduce the compilation time, we only compile the
            ...
            # the outputs.
            # FIXME: This is a temporary hack to not change the existing
...
class ModelWrapper(nn.Module):
    ...
    def forward(
        ...
        # FIXME: Disabled top-p sampling since it's too slow.
        # logits = _apply_top_p(logits, p.unsqueeze(dim=1))
        probs = torch.softmax(logits, dim=-1, dtype=torch.float32)
        # FIXME: best_of > 1 is not supported.
