...
from aphrodite.attention.backends.abstract import (AttentionBackend,
                                                   AttentionImpl,
                                                   AttentionMetadata)
from aphrodite.attention.ops.pallas_attention import \
...
class PallasAttentionBackendImpl(AttentionImpl):
    ...
    def __init__(
        ...
        if sliding_window is not None:
            raise NotImplementedError("Sliding Window attn is not supported.")
        if kv_cache_dtype != "auto":
            raise NotImplementedError("FP8 KV cache is not supported.")
        if blocksparse_params is not None:
            raise NotImplementedError("Blocksparse attn is not supported.")
