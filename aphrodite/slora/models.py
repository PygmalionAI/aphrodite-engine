...
from aphrodite.slora.layers import (LoRALayer, LoRAMapping, from_layer,
                                   from_layer_sampler)
...
class LoRAModelManager:
    ...
    def __init__(
        ...
        Args:
            ...
            max_num_seqs: the maximum number of sequences model can run
                in a single batch.
            max_num_batched_tokens: the maximum number of tokens model 
                can run in a single batch.
            vocab_size: the vocabulary size of the model.
            lora_config: the LoRA configuration.
            lora_target_modules: the target module patterns to be adapted.
                Support both single module name and a list of module names.
            packed_modules_mapping: the mapping for packed modules. Aphrodite
                packs some modules into one module, e.g. qkv_proj is packed
                of q_proj, k_proj, and v_proj. These modules have a single
                layer in the original model, but they are split into multiple
                layers in the adapted model.
        """
    ...
    def add_lora(self, lora: LoRAModel) -> bool:
        ...
        if lora.id not in self._registered_loras:
            if len(self._registered_loras) >= self.capacity:
                raise RuntimeError("No frtee LoRA slots!")
    ...
    def convert_mapping(self, mapping: LoRAMapping) -> None:
        ...
        self.base_indices[:indices_len[0]].copy_(base_indices)
        ...
        for module_name, module in self.model.named_modules():
            ...
            if module_name not  in self.packed_modules:
                if parts[-1] in EMBEDDING_MODULES:
                    ...
                    output_dim = module.base_layer.embedding_dim if hasttr(
                        module.base_layer,
