...
class ReorderPolicy(Policy):
    """
    ReorderPolicy tries to maximize throughput by reordering incoming requests
    by length.
    
    Args:
        reorder_window: window size in sec within which `List[SequenceGroup]`
            is allowed to be reordered. 0 means no reorder. 
    """
    ...
    def sort(
        ...
        """Sort head within `reorder_window` of the `seq_groups` by length.
        It reduces padding computation overhead."""
        if len(seq_groups) == 0:
            return seq_groups
    ...
    def get_preemption_mode(self, seq_group: SequenceGroup) -> PreemptionMode:
        """Always use SWAP, as it is faster than `RECOMPUTE` for heavy models
        like llama."""
        return PreemptionMode.SWAP
