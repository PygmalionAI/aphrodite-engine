...
async def get_gen_prompt(request) -> str:
    ...
    conv = Conversation(
        ...
        offset=conv.offset,
        sep_style=SeparatorStyle(conv.seq_style),
        sep=conv.sep,
        sep2=conv.sep2,
...
async def create_chat_completion(raw_request: Request):
    """ChatCompletion API similar to OpenAI's API
    
    See https://platform.openai.com/docs/api-reference/chat/create
    for the API specification. This API mimics the OpenAI ChatCompletion API.
    
    NOTE: Currently the following features are not supported:
        - function_call (users should implement this by themselves)
        - logit_bias (to be supported by aphrodite)
    """
    ...
    request_id = f"cmpl-{random_uuid()}"    # should this be `chatcmpl`?
    ...
    if request.stream:
        ...
        return StreamingResponse(completion_stream_generator(), media_type="text/event-stream", background=background_tasks)
