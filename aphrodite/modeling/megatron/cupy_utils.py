...
CUDA graphs. This is because torch.distributed.all_reduce causes errors when
...
class NCCLBackendWithBFloat16(NCCLBackend):
    ...
    # but broadcast will fail (will require changes in compiled cupy code).
...
def set_cupy_stream(stream: torch.cuda.Stream) -> None:
    """Set the cuda stream for comms."""
    cupy_stream = cupy.cuda.ExternalStream(stream.cuda_stream,
                                           stream.device.index)
    with cupy_stream:
        yield

def init_process_group(world_size: int, rank: int, host: str, port: int
) -> None:
    """Initialize the CuPy NCCL backend.
    ...
    if isinstance(cupy, Exception):
        raise ImportError(
            "NCCLBackend is not available. Please install CuPy with"
            "`pip install cupy-cuda11x`") from cupy
    ...
    assert 0 <= rank < world_size, (
        f"{rank=} should be an integer between [0, {world_size}]")
