...
from aphrodite.transformers_utils.configs import GrokConfig
...
class GrokMLP(nn.Module):
    ...
    def __init__(
        ...
        self.linear_v = ReplicatedLinear(
            self.hidden_dim,
            self.ffn_dim,
            bias=False,
            linear_method=linear_method,
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        linear_out, _ = self.linear(hidden_states)
        linear_out = self.act_fn(linear_out)
        linear_v_out, _ = self.linear_v(hidden_states)
        current_hidden_states = linear_out * linear_v_out
        current_hidden_states, _ = self.linear_1(current_hidden_states)
...
class GrokAttention(nn.Module):
    ...
    def forward(
        ...
        if self.merge_weight:
            ...
            k, _ = self.key(hidden_states)
            v, _ = self.value(hidden_states)
        q, k = self.rotary_emb(positions, q, k)
        k_cache, v_cache = kv_cache
        attn_output = self.attn(q, k, v, k_cache, v_cache, input_metadata)
        # NOTE: This is a hack, figure out a better way to do this.
        attn_output = 30 * torch.tanh(attn_output / 30)
        output, _ = self.linear(attn_output)
        return output
...
class GrokDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.block_sparse_moe = GrokMoE(config=config,
                                        linear_method=linear_method)
...
class GrokModel(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList([
            GrokDecoderLayer(config, linear_method=linear_method)
...
class GrokForCausalLM(nn.Module):
    ...
    def __init__(
        ...
        self.model = GrokModel(config, linear_method)
