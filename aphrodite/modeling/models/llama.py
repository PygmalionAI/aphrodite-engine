...
from aphrodite.common.sequence import SequenceOutputs
from aphrodite.quant import qmodule
...
class QuantLlamaMLP(nn.Module):
    def __init__(
        ...
        self.down_proj = get_quantized_layer(
            intermediate_size, hidden_size, quant_config)
        if hidden_act != "silu":
            raise ValueError(f"Unsupported activation: {hidden_act}. Only sily is currently supported.")
        self.act_fn = SiluAndMul()
...
class LlamaDecoderLayer(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, quant_config: QuantConfig):
        ...
        if use_quantized_layers:
            ...
            self.mlp = QuantLlamaMLP(hidden_size=self.hidden_size,
...
class LlamaForCausalLM(nn.Module):
    def __init__(self, config, quant_config):
        ...
        self.config = config
        self.quant_config: quant_config
        self.model = LlamaAttention(config, quant_config)
        vocab_size = ((config.vocab_size + 63) // 64) * 64
    ...
    def load_weights(self,
        ...
        for name, loaded_weight in hf_model_weights_iterator(
            ...
            for weight_name, shard_size, offset in attention_weight_specs:
                ...
                if not is_quantized:
                    loaded_weight = loaded_weight[shard_size * tensor_model_parallel_rank:shard_size *
