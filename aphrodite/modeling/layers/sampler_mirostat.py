...
def _fetch_args(
    ...
    for seq_ids, params in input_metadata.seq_groups:
        if params.mirostat_mode == 2:
            ...
            mus += [input_metadata.persistent_data.get(sid).get('miro_mu', params.mirostat_tau*2) for sid in seq_ids]
        index += len(seq_ids)
...
def _apply_mirostat_v2(
    ...
    mus: list[float],  # AKA the accumulator that always tries to approach [tau]
) -> Tensor:
    ...
    logit_surprise = torch.neg_(torch.log_softmax(logits, dim=-1))
    ...
    probs = torch.softmax(logits, dim=-1, dtype=logits.dtype) # Get probs, post-mask
    ...
    # print(f"   mus {mus[0]:>7.03f} |{miro_mask.logical_not().sum(dim=-1).item():>6} |{picked_surprises.item():>7.03f} |{eps.item():>7.03f} | {logit_surprise.min()} | {logit_surprise.max()}")
    tmus = tmus - tetas * eps
    mus[:] = tmus.tolist()
...
def is_applicable(input_metadata:InputMetadata) -> bool:
    return any((params.mirostat_mode == 2) for _,params in input_metadata.seq_groups)
...
def apply(
    ...
    logits[logit_index] = _apply_mirostat_v2(logits[logit_index], taus, etas, mus) # mus is an inout param, :vomit:
