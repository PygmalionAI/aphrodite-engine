...
def _top_k_top_p_multinomial_with_cuda_kernels(
    ...
    if not success.all():
        warnings.warn("CUDA kernel rejection sampling failed, fallback.",
                      stacklevel=1)
        probs = ops.top_k_renorm_prob(probs, top_ks)
        probs = ops.top_p_renorm_prob(probs, top_ps)
        batch_next_token_ids = ops.sampling_from_probs(probs,
                                                       uniform_samples[0])
