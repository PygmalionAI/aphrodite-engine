...
try:
    ...
    logger.warning("CUDA extension for AutoGPTQ not installed. "
                   "Only 4-bit GPTQ will work.")
    autogptq_cuda_256 = None
    autogptq_cuda_64 = None
    _autogptq_cuda_available = False

# The bit width to which the quantized weight is packed needs
# to align with your quantization code. The quantized parameters,
# represented in lower precision, are packed in int32 in GPTQ.
...
class GPTQConfig(QuantizationConfig):
    ...
    def __init__(self, weight_bits: int, group_size: int, desc_act: bool,
        ...
        # The Exllama kernel only supports 4 bits. Under 4-bit quantization,
        # it will be used if disable_exllama is False; otherwise, the Triton
    ...
    def create_weights(
        ...
        if self.quant_config.kernel_type == GPTQLinearKernel.CUDA:
            if self.quant_config.weight_bits in [2, 4, 8]:
                ...
                # Under 3-bit quantization, packing is different from other
                # cases because many 3 bits won't fit neatly into a 32-bit
                # width integer.
                # There could be one bit that spans across two 32-bit vectors.
                # Here, we use a tensor to track the offset of each bit in
                # three 32-bit vectors, which can neatly accommodate 32
                # 3-bit weights.
