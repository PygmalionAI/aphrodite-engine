...
def quant_matmul_248_kernel(
        ...
        stride_scales, stride_zeros, BLOCK_SIZE_M: tl.constexpr,
    ...
    for _ in range(0, num_pid_k):
        ...
        a = tl.load(a_ptrs, mask=a_mask,
                    other=0.)  # (BLOCK_SIZE_M, BLOCK_SIZE_K)
...
def quant_matmul_248(input, qweight, scales, qzeros, g_idx, bits, maxq):
    with torch.cuda.device(input.device):
        ...
        quant_matmul_248_kernel[grid](input, qweight, output,
                                      scales.to(input.dtype), qzeros, g_idx,
                                      input.shape[0], qweight.shape[1],
                                      input.shape[1], bits, maxq,
