...
class RayTokenizerGroupPool(BaseTokenizerGroup):
    ...
    def from_config(cls, tokenizer_pool_config: TokenizerPoolConfig,
        ...
        init_kwargs["num_workers"] = tokenizer_pool_config.pool_size
        init_kwargs["ray_actor_options"] = ray_actor_options

        return cls(**init_kwargs)
    
    def __init__(self, tokenizer_id: str, enable_lora: bool, max_num_seqs: int,
                 max_input_length: Optional[int], num_workers: int,
    ...
    def encode(
        ...
        if self._idle_actors.empty():
            raise RuntimeError("All actors are busy.")
        ...
        try:
            ...
            # This is done in a finally block to ensure
            # that the actor is always put back in the queue,
...
def _carry_over_env_vars_to_runtime_env(runtime_env: dict) -> None:
    """Copy over all current process environment variables to the
    runtime_env.
    
    The variables in runtime_env will take precedence over the current
