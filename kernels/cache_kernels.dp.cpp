...
void swap_blocks(
  ...
  for (const auto& pair : block_mapping) {
    ...
    dpct::async_dpct_memcpy(dst_ptr + dst_offset,
      src_ptr + src_offset,
      block_size_in_bytes,
      memcpy_type,
      stream);
...
void copy_blocks(
  ...
  sycl::range<3> block(std::min(1024, numel_per_block));
  const dpct::queue_ptr stream = at::cuda::getCurrentCUDAStream();
  stream.submit([&](sycl::handler &cgh){

  cgh.parallel_for(sycl::nd_range<3>(grid * block, block),
   [=](sycl::nd_item<3> item_ct1){
   aphrodite::copy_blocks_kernel<scalar_t>(
  ...
  torch::Tensor& slot_mapping)  // [num_tokens]
{
  ...
  cgh.parallel_for(sycl::nd_range<3>(grid * block, block),
   [=](sycl::nd_item<3> item_ct1){
   aphrodite::reshape_and_cache_kernel<scalar_t>(
