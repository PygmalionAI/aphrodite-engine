...
void paged_attention_v1_dispatch(
  ...
  APHRODITE_DISPATCH_DEVICES(out.device(), paged_attention_v1, out, query, key_cache, value_cache, num_kv_heads, scale, block_tables, context_lens, block_size, max_context_len, alibi_slopes);
}
...
void paged_attention_v2_dispatch(torch::Tensor &out, torch::Tensor &exp_sums,
    ...
    int max_context_len, const c10::optional<torch::Tensor> &alibi_slopes) {
  APHRODITE_DISPATCH_DEVICES(out.device(), paged_attention_v2, out, exp_sums, max_logits, tmp_out, query, key_cache, value_cache, num_kv_heads, scale, block_tables, context_lens, block_size,max_context_len, alibi_slopes);
}
...
torch::Tensor gptq_gemm_dispatch(torch::Tensor a, torch::Tensor b_q_weight, torch::Tensor b_gptq_qzeros, torch::Tensor b_gptq_scales, torch::Tensor b_g_idx, bool use_exllama) {
  APHRODITE_DISPATCH_DEVICES(a.device(), gptq_gemm, a, b_q_weight, b_gptq_qzeros, b_gptq_scales, b_g_idx, use_exllama);
}

torch::Tensor gptq_shuffle_dispatch(torch::Tensor q_weight, torch::Tensor q_perm) {
  APHRODITE_DISPATCH_DEVICES(q_weight.device(), gptq_shuffle, q_weight, q_perm);
}
...
void reshape_and_cache_dispatch(torch::Tensor& key, torch::Tensor& value, torch::Tensor& key_cache, torch::Tensor& value_cache, torch::Tensor& slot_mapping) {
  APHRODITE_DISPATCH_DEVICES(key.device(), reshape_and_cache, key, value, key_cache, value_cache, slot_mapping);
}
