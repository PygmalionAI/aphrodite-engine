...
#define CALL_RESHAPE_AND_CACHE(KV_T, CACHE_T, KV_DTYPE)                                     \
  aphrodite::reshape_and_cache_kernel<KV_T, CACHE_T, KV_DTYPE><<<grid, block, 0, stream>>>(      \
  ...
  const std::string& kv_cache_dtype)
{
  ...
  APHRODITE_DISPATCH_FLOATING_TYPES(
    ...
    [&] {
      aphrodite::reshape_and_cache_flash_kernel<scalar_t><<<grid, block, 0, stream>>>(
  ...
  const std::string& kv_cache_dtype)
{
  ...
  if (kv_cache_dtype == "auto") {
    ...
    if (src_cache.dtype() == at::ScalarType::Float) {
      ...
      CALL_CONVERT_FP8(uint8_t, __nv_bfloat16, aphrodite::Fp8KVCacheDataType::kFp8E4M3);
    } else if (dst_cache.dtype() == at::ScalarType::Float) {
      ...
      CALL_CONVERT_FP8(__nv_bfloat16, uint8_t, aphrodite::Fp8KVCacheDataType::kFp8E4M3);
    }
  } else {
