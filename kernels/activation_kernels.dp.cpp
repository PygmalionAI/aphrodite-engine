  ...
  torch::Tensor& input)    // [..., 2 * d]
{
  ...
  cgh.parallel_for(sycl::nd_range<3>(grid * block, block),
   ...
   aphrodite::silu_and_mul_kernel<scalar_t>(out.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(),d, item_ct1);
   });
...
#define LAUNCH_ACTIVATION_KERNEL(KERNEL)                                       \
  ...
  cgh.parallel_for(sycl::nd_range<3>(grid * block, block),                     \
   [=](sycl::nd_item<3> item_ct1){                                             \
   aphrodite::activation_kernel<scalar_t, KERNEL<scalar_t>>(out.data_ptr<scalar_t>(),\
