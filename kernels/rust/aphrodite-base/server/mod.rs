use crate::{
    ...
    seq::RequestOutput,
    util::apply_settings,
    AddRequest, HashMap, LoaderArgs, ModelExec, AphroditeEngine,
};
...
fn run_tests<ME: ModelExec>(
    args: &AphroditeCliArgs,
    loader_args: LoaderArgs,
    model_args: ME::ModelLoaderArgs,
) {
    let mut engine = ME::load_aphrodite_engine(loader_args, model_args).expect("failed to load model");
...
fn spawn_inference_loop<ME: ModelExec>(
    ...
    std::thread::spawn(move || {
        set_max_priority();
        let mut engine =
            ME::load_aphrodite_engine(loader_args, model_args).expect("failed to load model");
...
pub async fn server_main<ME: ModelExec>(
    ...
    let (tokenizer, tok_trie) =
        AphroditeEngine::<ME>::load_tokenizer(&mut loader_args).expect("failed to load tokenizer");
