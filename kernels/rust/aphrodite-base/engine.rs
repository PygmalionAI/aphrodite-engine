use crate::{
    config::{ParallelConfig, AphroditeConfig, SamplingParams, SchedulerConfig},
    iface::AiciRtIface,
    seq::{
...
pub struct AphroditeEngine<ME: ModelExec> {
    pub config: Arc<AphroditeConfig<ME>>,
...
impl<ME: ModelExec> AphroditeEngine<ME> {
    pub fn build_config(
        args: &LoaderArgs,
        model_args: &mut ME::ModelLoaderArgs,
    ) -> Result<AphroditeConfig<ME>> {
        ...
        let aphrodite_config = AphroditeConfig {
            model: model_config,
            meta: model_meta,
        ...
        ME::verify_args(&aphrodite_config)?;
    ...
    pub fn build(
        ...
        aphrodite_config: Arc<AphroditeConfig<ME>>,
    ) -> Result<Self> {
        let (tokenizer, tok_trie) = AphroditeEngine::<ME>::load_tokenizer(&mut args)?;
