use super::{
    config::{self, TchAphroditeConfig},
    loader::{load_model_config, load_aphrodite_engine},
    attention::{BatchInfo, BatchInfoBuilder, BlockSpaceManager, CacheEngine, CacheIface, TchSeqMgr},
...
use aphrodite::{config::AphroditeConfig, AiciBias, LogitsProcessor, ModelExec, SchedulerOutputs};
...
pub struct TModel {
    config: Arc<AphroditeConfig<TModel>>,
...
impl ModelExec for TModel {
    ...
    fn load_model_config(
        args: &aphrodite::LoaderArgs,
        model_args: &mut Self::ModelLoaderArgs,
    ) -> Result<(aphrodite::config::ModelMeta, Self::ModelConfig)> {
        let m = load_model_config(args, model_args)?;
        Ok((m.meta.clone(), m))
    }

    fn verify_args(args: &AphroditeConfig<Self>) -> Result<()> {
        args.verify_args()
    }

    fn load_aphrodite_engine(
        args: aphrodite::LoaderArgs,
        model_args: Self::ModelLoaderArgs,
    ) -> Result<aphrodite::AphroditeEngine<Self>> {
        load_aphrodite_engine(args, model_args)
    }
