...
struct Selective_Scan_fwd_kernel_traits {
  ...
  using BlockLoadWeightT =
      cub::BlockLoad<input_t, kNThreads, !kIsComplex ? kNItems : kNItems * 2,
                     cub::BLOCK_LOAD_WARP_TRANSPOSE>;
  using BlockLoadWeightVecT =
      cub::BlockLoad<vec_t, kNThreads, !kIsComplex ? kNLoads : kNLoads * 2,
                     !kDirectIO ? cub::BLOCK_LOAD_WARP_TRANSPOSE
                                : cub::BLOCK_LOAD_DIRECT>;
...
#pragma unroll
      ...
      if constexpr (kIsVariableB) {
        load_weight<Ktraits>(
            Bvar + state_idx * params.B_dstate_stride, B_vals, smem_load_weight,
            (params.seqlen - chunk * kChunkSize) * (!kIsComplex ? 1 : 2));
...
#pragma unroll
      ...
      if constexpr (kIsVariableC) {
        ...
        load_weight<Ktraits>(
            Cvar + state_idx * params.C_dstate_stride, C_vals,
            smem_load_weight_C,
            (params.seqlen - chunk * kChunkSize) * (!kIsComplex ? 1 : 2));
        if constexpr (!kIsVariableB) {
#pragma unroll
...
#pragma unroll
        for (int i = 0; i < kNItems; ++i) {
          if constexpr (!kIsComplex) {
            ...
            // Pytorch's implementation of complex exp (which calls thrust) is
            // very slow
            ...
            weight_t B_delta_u_val = !kIsVariableB
                                         ...
                                         : B_vals[i] * delta_u_vals[r][i];
            thread_data[i] =
                make_float4(delta_a_exp.real_, delta_a_exp.imag_,
                            B_delta_u_val.real_, B_delta_u_val.imag_);
            if constexpr (!Ktraits::kIsEvenLen) {  // So that the last state is
                                                   // correct
              if (threadIdx.x * kNItems + i >=
                  params.seqlen - chunk * kChunkSize) {
        ...
        if constexpr (!kIsComplex) {
          // If we use WARP_SCAN then all lane 0 of all warps (not just thread
          // 0) needs to read
          ...
          // running_prefix = chunk > 0 && threadIdx.x == 0 ?
          // smem_running_prefix[state_idx] : make_float4(1.f, 0.f, 0.f, 0.f);
        }
      ...
      BOOL_SWITCH(params.is_variable_C, kIsVariableC, [&] {
        BOOL_SWITCH(params.z_ptr != nullptr, kHasZ, [&] {
          using Ktraits = Selective_Scan_fwd_kernel_traits<
              ...
              kIsVariableC, kHasZ, input_t, weight_t>;
