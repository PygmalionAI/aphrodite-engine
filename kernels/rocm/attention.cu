...
template <typename T, aphrodite::Fp8KVCacheDataType KV_DTYPE>
__device__ __forceinline__ _B16x8 scaled_convert_b8x8(const _B8x8 input,
                                                      const float scale) {
  union alignas(16) {
    uint4 u4;
    _B16x8 u16x8;
    aphrodite::bf16_8_t b16x8;
  } tmp;
  if constexpr (std::is_same<T, _Float16>::value) {
    tmp.u4 =
        aphrodite::fp8::scaled_convert<uint4, _B8x8, KV_DTYPE>(input, scale);
    return tmp.u16x8;
  } else if constexpr (std::is_same<T, __hip_bfloat16>::value) {
    tmp.b16x8 =
        aphrodite::fp8::scaled_convert<aphrodite::bf16_8_t, _B8x8, KV_DTYPE>(
            input, scale);
    return tmp.u16x8;
  } else {
    static_assert(false, "unsupported 16b dtype");
