...
{
  int4 cumulative_sizes = aphrodite::aqlm::accumulate_sizes(codebook_partition_sizes);
  ...
  { 
    return aphrodite::aqlm::code1x16_matmat(input, codes, codebooks, scales, cumulative_sizes, bias);
  }
  if (nbooks == 2 && entries == (1 << 8))
  {
    return aphrodite::aqlm::code2x8_matmat(input, codes, codebooks, scales, cumulative_sizes, bias);
  }
