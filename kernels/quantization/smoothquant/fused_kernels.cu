...
void dequant_add_residual(
  ...
  APHRODITE_DISPATCH_FLOATING_TYPES(
      residual.scalar_type(), "dequant_add_residual_kernel", [&] {
        aphrodite::dequant_add_residual_kernel<scalar_t, false><<<grid, block, 0, stream>>>(
...
void dequant_add_residual(
  ...
  APHRODITE_DISPATCH_FLOATING_TYPES(
      residual.scalar_type(), "dequant_add_residual_kernel", [&] {
        aphrodite::dequant_add_residual_kernel<scalar_t, true><<<grid, block, 0, stream>>>(
          input.data_ptr<int32_t>(),
          residual.data_ptr<scalar_t>(),
          out.data_ptr<scalar_t>(),
...
void dequant(
  ...
  const cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  APHRODITE_DISPATCH_FLOATING_TYPES(out.scalar_type(), "dequant_kernel", [&] {
    aphrodite::dequant_kernel<scalar_t, false><<<grid, block, 0, stream>>>(
      input.data_ptr<int32_t>(),
      out.data_ptr<scalar_t>(),
      scale,
      num_tokens,
...
void dequant(
  ...
  const cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  APHRODITE_DISPATCH_FLOATING_TYPES(out.scalar_type(), "dequant_kernel", [&] {
    aphrodite::dequant_kernel<scalar_t, true><<<grid, block, 0, stream>>>(
      input.data_ptr<int32_t>(),
      out.data_ptr<scalar_t>(),
      weight_dequant_scale,
      num_tokens,
...
void quant(
  ...
  dim3 block(std::min(hidden_size, 1024));
  const cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  APHRODITE_DISPATCH_FLOATING_TYPES(input.scalar_type(), "quant_kernel", [&] {
    aphrodite::quant_kernel<scalar_t, float, false><<<grid, block, 0, stream>>>(
...
void quant(
  ...
  const cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  APHRODITE_DISPATCH_FLOATING_TYPES(input.scalar_type(), "quant_kernel", [&] {
    aphrodite::quant_kernel<scalar_t, float*, true><<<grid, block, 0, stream>>>(
      input.data_ptr<scalar_t>(),
      out.data_ptr<int8_t>(),
      scale.data_ptr<float>(),
      num_tokens,
