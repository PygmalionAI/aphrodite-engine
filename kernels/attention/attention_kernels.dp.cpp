...
#pragma unroll
  ...
  performance if there is no access to global memory.
  */
  item_ct1.barrier(); // TODO: possible speedup if this is replaced with
   ...
   stream->submit([&](sycl::handler &cgh) {                                    \
      ...
      cgh.parallel_for(                                                        \
          ...
          [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {  \
             aphrodite::paged_attention_v1_kernel<T, HEAD_SIZE, BLOCK_SIZE,         \
                                             NUM_THREADS>(                     \
...
#define LAUNCH_PAGED_ATTENTION_V2(HEAD_SIZE)                                   \
   stream->submit([&](sycl::handler &cgh) {                                    \
      ...
      cgh.parallel_for(                                                        \
          sycl::nd_range<3>(grid * block, block),                              \
          [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {  \
             aphrodite::paged_attention_v2_kernel<T, HEAD_SIZE, BLOCK_SIZE,         \
